<template>
  <vc-blade
    ...
    :searchable="true"
  >
    <vc-table :headers="headers" :items="items" :multiselect="true" @itemClick="openDetails($event)">
      ...
      <template v-slot:item_name="itemData">
        <div class="vc-flex vc-flex vc-flex-align_center vc-fill_width">
          <vc-icon :icon="itemData.item.icon" size="xl" style="color: #a5a5a5"></vc-icon>
          <div class="vc-font-size_m vc-font-weight_bold vc-flex-grow_1 vc-margin-horizontal_m">
            {{ itemData.item.name }}
          </div>
...
<script>
  export default {
    ...
    methods: {
      openDetails(blade) {
        this.$store.commit("openBlade", {
          component: () => import("@/components/blade/StoreDetails.vue"),
        });
      },
    },
  };
