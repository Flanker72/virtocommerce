  ...
  <div class="vc-layout vc-flex vc-fill_all">
    ...
    <div class="vc-flex vc-flex-grow_1 vc-flex-column">
      <div class="vc-layout__topbar vc-flex vc-fill_width vc-flex-align_stretch vc-flex-shrink_0">
        ...
        <div v-if="$slots['notification']" class="vc-layout__topbar-notification">
        ...
        <div v-if="toolbarItems" class="vc-layout__topbar-toolbar vc-flex">
          <div
            ...
            :key="item.id"
            class="vc-layout__topbar-toolbar-item vc-flex vc-flex-align_center vc-fill_height vc-flex-justify_center"
            :class="{ 'vc-layout__topbar-toolbar-item_accent': item.accent }"
            :title="$t(item.title)"
            @click="item.hasOwnProperty('onClick') ? item.onClick() : null"
          >
            <vc-icon :icon="typeof item.icon === 'function' ? item.icon() : item.icon" size="xl"></vc-icon>
          </div>
        </div>
        ...
        >
          ...
          <div class="vc-layout__topbar-account-menu" v-if="accountMenuVisible" @click.stop>
            <div class="vc-layout__topbar-account-menu-item">{{ $t("Profile") }}</div>
            <div class="vc-layout__topbar-account-menu-item">{{ $t("Log Out") }}</div>
          </div>
        </div>
      </div>
